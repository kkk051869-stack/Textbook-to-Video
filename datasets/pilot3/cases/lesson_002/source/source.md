...
- candidate_id: `docx_ch08_s03`
- source_locator: `docx:chapter=8;section=3`
- text_chars: 1788
- image_count: 1

## Images

- `fig7-6`: 区块链数据结构 -> `images/fig7-6_区块链数据结构.png`
...
哈希函数是密码学中的重要手段，可以依据一串输入数据创建一个简短的哈希散列值，被广泛用于验证数据的真实性。输入数据即使发生微小变动，也会导致输出信息发生显著变化；另外，也无法基于输出的哈希散列值反推出原始数据。因此，在每个区块链中保留的“上一区块的哈希散列值”就可反应出上一区块内的数据是否曾被篡改过了。例如，假设在某个区块中存在这样一笔交易，张三将100元钱转账给李四。如果某个参与者想偷偷把转账金额调高至500元，则这个行为很快就能够被发现，因为基于原有区块的哈希散列值已经被分布式记账了，无法在不改变上一区块的哈希散列值的情况下修改交易记录。
图7-6 区块链数据结构
（二）区块链的缘起
