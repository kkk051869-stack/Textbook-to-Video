...
同一类目下有多个 widget，同页并用会**重复啰嗦**。每组的 **widget 形式**最多选 1 种：

- **列举组**：`icon_group`（列要点）/ `flow_step`（列步骤）/ `activity_step`（列操作）——选 1 种。
- **数据展示组**：`comparison_panel` / `table` / `bar` / `chart_line`——选 1 种。
- **小标签组**：`badge` / `label`——选 1 种。

**⚠️ 取一是"少用一个 widget"，绝不是"丢信息"。** 信息完整 > 避免重复 > 类型精简。当一页本想用同组两个 widget 时，按下面处理：

1. **融进所选 widget**：如既要"流程"又要"各步特征"，就把特征写进 `flow_step` 每步的文字，而非再加 `icon_group`。
2. **次要信息用不重叠的类型承载**：列举组只用 1 个 widget，但次要要点可放进 `text` / `quote`（不属列举组），不算重复。
3. **两件独立的事 → 该段信息过载，应拆成两段**：若一页同时有"一条流程"和"一组并列要点"两件**互不从属**的内容，本质是两个讲稿点——宁可标记内容偏多、让上游拆成两页，也不要挤一页或删内容。
