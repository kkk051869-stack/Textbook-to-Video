...
EVALUATOR_VERSION = "semantic-timing-v0.1"
...
def _runtime_metrics(
    ...
    *,
    ...
    by_event_id: dict[tuple[int, str], list[tuple[int, dict[str, Any]]]] = {}
    by_key: dict[tuple[int, str], list[tuple[int, dict[str, Any]]]] = {}
    for index, event in enumerate(events):
        event_id = event.get("event_id")
        slide = event.get("slide", event.get("slide_id"))
        if isinstance(slide, bool) or not isinstance(slide, int):
            continue
        if event_id is not None:
            by_event_id.setdefault((slide, str(event_id)), []).append((index, event))
    ...
    for row in rows:
        ...
        candidates = (
            by_event_id.get((int(row["slide_id"]), str(row.get("event_id"))), [])
    ...
    # animation_trace.runtime_errors is a summary of the same event stream in
    # the public adapter contract.  Use it only to cover extra summaries; do
    # not double-count an error already represented by an event.
    counts["runtime_error_count"] = max(
...
def build_semantic_timing_report(
    ...
    *,
    ...
    runtime_details: dict[str, Any] = {
        ...
        "trace_join_key": "event_id, else unique (slide index, element target)",
...
def evaluate_semantic_timing(context: Any) -> dict[str, Any]:
    ...
    report["details"] = {
        ...
        "evaluated_elements": report["evaluated_elements"],
        "provenance": {
            "evaluator_version": EVALUATOR_VERSION,
            "candidate_commit": getattr(context, "candidate_commit", None),
