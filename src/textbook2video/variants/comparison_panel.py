...
def _item_content(item) -> str:
    if isinstance(item, dict):
        return str(item.get("content") or item.get("body") or "")
    return str(item or "")
...
def _cp_panel(item: dict, accent: str) -> str:
    ...
    return (
        ...
        f'margin-bottom:14px;">{_esc(_item_title(item))}</div>'
        f'<div style="font-size:{_fs(22)};line-height:1.6;color:var(--text-dim);">'
        f'{_esc(_item_content(item))}</div></div>'
    )
...
def _cmp_stacked_rows(elem, d, seg_id, _imgs) -> str:
    ...
    def row(item, accent):
        return (
            ...
            f'{_esc(_item_title(item))}</div>'
            f'<div style="flex:1;font-size:{_fs(22)};line-height:1.55;'
            f'color:var(--text-dim);">{_esc(_item_content(item))}</div></div>'
        )
    return (
...
def _cmp_v_split_dashed(elem, d, seg_id, _imgs) -> str:
    ...
    def col(item):
        return (
            ...
            f'font-weight:500;">{_esc(_item_content(item))}</div></div>'
        )
    return (
        f'<div class="anim anim-card {d}" style="display:flex;align-items:stretch;'
...
def _cmp_top_bottom_compare(elem, d, seg_id, _imgs) -> str:
    ...
    def section(item, accent, tag):
        return (
            ...
            f'margin-bottom:10px;">{_esc(_item_title(item))}</div>'
...
def _cmp_chart_bar(elem, d, seg_id, _imgs) -> str:
    ...
    lens = [min(100, max(60, len(_item_content(it)) * 2.5)) for it in items]
    def bar(item, accent, width_pct):
        return (
            ...
            f'<span style="font-size:{_fs(24)};">{_esc(_item_title(item))}</span>'
            f'</div>'
            ...
            f'padding:6px 8px 0;">{_esc(_item_content(item))}</div></div>'
        )
    return (
        f'<div class="anim anim-card {d}" style="display:flex;flex-direction:column;'
