...
- `quiz_card` 现在先显示题目，再显示答案和解析。
- 答案解析区带 `data-step="1"`，录制视频时会按 slide 时长自动揭示。
- HTML 里新增“显示答案”按钮，预览或手动打开 HTML 时可以点击。
- slide controller 新增 quiz reveal 逻辑。
- 已补测试，检查按钮和 reveal 标记存在。

为什么重要：

- 知识点检测页开始有“先思考，再看答案”的教学节奏。
- 视频输出里能自动展示答案解析，人工预览时也能手动点开。
- 这是从“静态检测题”走向“交互式教学活动”的第一步。

怎么看成果：

生成 HTML 后，`quiz_card` 会包含：

```html
<button data-quiz-action="reveal">显示答案</button>
<div data-quiz-reveal="1" data-step="1">...</div>
```

还没做到什么：

- 目前是显示答案，不是选择题点击作答和自动判分。
- 还没有单独的暂停策略配置，例如“题目页停 5 秒再揭示”。

## 下一版计划：选择题作答 / 教材图区域自动定位

下一版可以从两个方向选一个：

- 继续教学闭环：让知识点检测页支持选择题作答和自动判分。
