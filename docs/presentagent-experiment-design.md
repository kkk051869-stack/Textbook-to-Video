...
> 文档版本：v2.0，双人并行版
> 数据集：TextbookEval-v1
> 内部系统基线：正式实验前冻结，以 `experiment/textbookeval-v1-freeze` 分支或 tag、commit、配置文件指纹和 prompt 文件指纹为准
...
这里必须区分“内容生成”和“视频导出”。`C00/C01` 必须来自同一个 P0 内容分支，`C10/C11` 必须来自同一个 P1 内容分支。静态版和定时版只能改变页内元素出现方式，不能分别重新生成讲稿、分镜、图片、旁白或字幕。

全部 96 个内部视频都计算自动指标；运行可靠性按 48 次内容生成和 96 个视频导出分别记录。这样既能考察平均质量和运行稳定性，又不会破坏静态版与定时版的配对控制。

PresentAgent 外部基线每个正式课节运行 1 次。由于它不做 3 次重复，本实验只将它作为按课节配对的外部参考系统，不用它比较生成稳定性。若试运行发现 PresentAgent 输出波动过大，再单独增加重复次数并冻结新的运行矩阵。
...
| 执行 48 次内部内容生成并导出 96 个内部视频 | 执行 8 次外部完整运行 | A 批量生成 P0/P1 内容分支，并为每个分支导出静态版和定时版；B 为 8 个正式课节各生成 1 个 PresentAgent 结果。 |
...
建立数据集不是简单把教材文件放到一个目录里，而是为每个课节准备一套公平的“考试材料”和“参考答案”。所有系统都只能看到同一份教材输入；评分时再使用隐藏的人工标注和评价题判断生成结果好不好。
...
```text
sources/<lesson_id>/
├── source.json          # 机器读取的标准正文和图片信息
├── source.md            # 人能直接阅读的教材正文
├── source.pdf           # 给 PresentAgent 使用的输入 PDF
├── images/              # 本课节的教材图片
├── annotation.json      # 人工标准标注，即评分参考答案
├── questions.json       # 保留评价题，不能给生成系统看
└── checksums.sha256     # 文件指纹，防止冻结后内容变化
```

`source.json`、`source.md`、`source.pdf` 和 `images/` 是生成系统可以使用的输入材料。`annotation.json` 和 `questions.json` 只用于评价，不能进入任何生成提示词、lesson plan、storyboard 或 PresentAgent 输入。

### 7.2 先做两个试运行课节

第一轮不要直接建设 8 个正式课节。先准备 2 个试运行课节：
...
标准课节包
  -> 我们系统 P0/P1 内容分支
  -> C00/C01/C10/C11 四个视频
  -> 静态/定时配对一致性检查
  -> quality report
  -> PresentAgent 最小对照
```

两个试运行课节都通过后，才扩展到 8 个正式实验课节。

### 7.3 整理教材正文

从 DOCX 或 PDF 中提取本课节正文时，只保留教材原文，不提前总结、不改写、不加入教学计划、不加入评价题。

`source.md` 用于人工复核，建议格式：
...
```text
required：必须使用，通常是教材中承载核心知识的图。
optional：可以使用，但不用也不影响核心讲解。
exclude：不进入生成，例如二维码、装饰图、无关图。
```

如果图片无法可靠提取或无法判断含义，该课节暂不进入正式实验。

### 7.5 给 PresentAgent 准备 `source.pdf`

`source.pdf` 从同一个标准课节包导出，供 PresentAgent 使用。它只包含：
...
- 评价题；
- 答案；
- 我们系统生成的讲稿或 storyboard。

导出 `source.pdf` 后，需要重新抽取 PDF 文本，并与 `source.json` 中的规范化正文计算一致性。正文或图片集合不一致时，不允许进入生成。

### 7.6 人工标准标注

人工标准标注必须在观看任何系统输出之前完成。每个课节至少标：
...
```json
{
  ...
  "knowledge_points": [
    {
      "id": "kp01",
      "statement": "知识驱动范式通过显式知识表示和推理规则解决问题。",
      "importance": 3,
      "evidence": ["p002", "p003"],
      "required_images": [],
  ...
  "image_requirements": [
    {
      "image_id": "img01",
      "required": true,
      "should_explain": "图中不同范式之间的关系",
      "related_knowledge_points": ["kp02"]
    }
  ],
  "misconceptions": [
    {
      "id": "m01",
      "text": "把人工智能范式理解成单一技术路线。",
      "related_knowledge_points": ["kp01"]
    }
  ]
}
```
...
每课准备 8 道评价题，单独保存为 `questions.json`。这些题只用于评价，不能提供给生成系统。
...
一旦课节包冻结，后续不能因为某个系统生成得不好而修改正文、图片、知识点或评价题。确实发现教材提取错误时，必须记录修改原因，重新生成文件指纹，并让 A/B 双方重新确认。
...
- 从 DOCX 中列出候选课节；
- 选择 DOCX 试运行课节；
- 导出 `source.json`、`source.md` 和 `images/`；
- 检查图片和段落位置是否对应；
- 准备我们系统可读取的输入；
- 复核 B 制作的 PDF 课节包。

B 负责 PDF 课节、PresentAgent 输入和评价格式：

- 从 PDF 中列出候选课节；
- 选择 PDF 试运行课节；
- 导出 `source.json`、`source.md`、`images/` 和 `source.pdf`；
- 制定 `annotation.json` 和 `questions.json` 的字段规则；
- 检查 PresentAgent 是否能读取 `source.pdf`；
- 复核 A 制作的 DOCX 课节包。
...
第一轮只做一个 DOCX 试运行课节的最小闭环：

```text
sources/docx_ch08_s01/
├── source.json
├── source.md
├── source.pdf
├── images/
...
这个闭环跑通后，再制作第二个试运行课节，并测试 PresentAgent 最小对照。两个试运行课节都通过后，再建设正式 8 个课节。
...
- 定时逐步展示版保留同一 storyboard 中的 `trigger_at_sec`，并在 HTML 中注入 `slideTimelines`。
- 静态展示版使用同一 storyboard、同一音频、同一字幕和同一页面时长，但所有页内元素在页面进入时立即可见。
- 静态展示版可以移除或覆盖元素级延迟动画，但不能改动文字、图片、页面数量、旁白、字幕、音频和 `slideDurations`。
- 静态版和定时版必须由同一个 `content_manifest.json` 派生，不能分别调用完整生成流程。
...
```text
scripts/verify_static_timed_pair.py
```
...
配对一致性检查必须在正式评分前自动执行，并把结果写入：

```text
runs/internal/<lesson_id>/<pair_id>/rXX/pair_consistency.json
```
...
内部 `formal_content_matrix.csv` 共 48 行，每行代表一次内容生成：
...
内部 `formal_video_matrix.csv` 共 96 行，每行代表一个由内容分支导出的视频：

下面这些英文是 CSV 的实际列名，分别记录视频编号、课节、实验条件、是否使用教学计划、是否定时逐步展示、重复次数、对应内容分支、执行状态以及它是否被选为标准评分结果。
...
```text
experiments/textbookeval-v1/configs/freeze-manifest.json
```

至少记录：

```text
textbook_to_video_branch_or_tag
textbook_to_video_commit
presentagent_commit
...
| 第 5 周 | 48 次内部内容生成并导出 96 个内部视频 | 8 次 PresentAgent 运行、评分准备 | 生成产物齐备 |
...
在这些文件完成并通过交叉检查前，不开始正式 48 次内容生成和 96 个视频导出。
