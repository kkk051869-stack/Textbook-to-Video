...
| 角色 | 主责链路 | 辅助链路 | 核心交付 |
| --- | --- | --- | --- |
| A | Textbook-to-Video 生成链路 | 运行日志、失败案例、自动指标 | 内部系统 16 个正式视频及完整运行记录 |
...
- `C01` 使用 `P0` 的讲稿、分镜、音频和原始 `trigger_at_sec`；
- `C11` 使用 `P1` 的讲稿、分镜、音频和原始 `trigger_at_sec`；
- 两个条件除 lesson plan 开关外，模型、主题、TTS、浏览器和录制配置必须一致；
- 正式评估每个条件只生成一次，编号为 `formal`；技术失败时按预设重试规则重跑，不允许按观感选择结果。

正式规模：

```text
8 课节 × 2 内容分支 = 16 次内部内容生成
16 次内容生成 = 16 个内部视频
```

如需研究随机性或运行稳定性，在主评估完成后另选 1 至 2 个代表课节运行 `r01`、`r02`、`r03`。必须记录 seed、采样参数和运行顺序；这些结果只用于稳定性分析，不进入主评分或与 PresentAgent 的主比较。
...
```text
runs/internal/{lesson_id}/{branch}/{condition}/
```

示例：

```text
runs/internal/lesson_001/P0/C01/
runs/internal/lesson_001/P1/C11/
```
...
| `lesson_id` | 课节编号 |
| `branch` | `P0` 或 `P1` |
| `run_kind` | `formal` 或 `stability` |
| `attempt` | `formal`、`retry01`，或稳定性运行的 `r01`、`r02`、`r03` |
...
正式评分使用 `formal`。若其因可记录的技术原因失败，使用第一个按既定配置成功完成的 `retryNN`，并在日志和评分表中说明替代原因；不得因内容或视觉观感重跑、替换或挑选结果。
...
也就是每课 3 个视频，8 课共 24 个视频用于人工主评价。

若 `formal` 技术失败，使用预先记录的第一个成功 `retryNN`，并在评分表中标注替代原因；不允许按观感选择结果。
...
4. A/B 用 `lesson_001` 的 3 个代表性视频试评一次，修正评分表。
