...
> ⚠️ **方法论已被 `adaptive-slide-layout.md` 取代**。本文的问题分析（尤其元素堆叠致溢出）仍有参考价值，
> 但其"Python 用字数估高度 → 套固定模板 → 压缩"的解法方向有误（实测证明同类元素高度方差极大，估算不可行）。
> 文中"实测 7/8 PASS"对应的 `estimate_height`/`_choose_layout` 原型**从未合入 master**。请以新文档为准。
