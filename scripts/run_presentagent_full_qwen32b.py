...
ZH_LANGUAGE_ADAPTATION = (
    "PresentAgent role prompts minimally adapted to use Simplified Chinese for "
    "user-facing slide text, outlines, section names, and speaker notes while "
    "keeping JSON keys, API names, code, paths, and schema fields unchanged"
)
...
def patch_presentagent_for_text_qwen(language: str = "none"):
    import re

    import torch
    import pptagent.agent as agent_mod
    ...
    original_agent_init = agent_mod.Agent.__init__
    ...
    def adapt_prompt_to_chinese(text: str) -> str:
        text = text.replace("You must use English.", "")
        text = text.replace("you must use English.", "")
        text = text.replace("Output: give your output in JSON format, you must use English.", "Output: give your output in JSON format.")
        language_rule = (
            "\n\nLanguage rule for this experiment: use Simplified Chinese for all "
            "user-facing slide content, outlines, section names, summaries, and "
            "speaker notes. Preserve required JSON keys, API names, Python/code "
            "tokens, file paths, and schema field names exactly as specified."
        )
        return text.rstrip() + language_rule

    def agent_init_with_language(self, *args, **kwargs):
        original_agent_init(self, *args, **kwargs)
        if language != "zh":
            return
        self.system_message = adapt_prompt_to_chinese(self.system_message)
        if isinstance(self.config, dict) and isinstance(self.config.get("template"), str):
            self.config["template"] = adapt_prompt_to_chinese(self.config["template"])
    ...
    agent_mod.Agent.__init__ = agent_init_with_language
...
async def run_one(modules: dict, lesson_id: str, args: argparse.Namespace) -> int:
    ...
    task_id = f"{args.run_prefix}/{lesson_id}"
    ...
    elif args.language == "zh" and args.force:
        (pdf_dir / "refined_doc.json").unlink(missing_ok=True)
    ...
    status = {
        ...
        "adaptations": ADAPTATIONS + ([ZH_LANGUAGE_ADAPTATION] if args.language == "zh" else []),
...
async def main() -> int:
    ...
    parser.add_argument("--language", choices=["none", "zh"], default="none")
    parser.add_argument("--run-prefix", default="full-qwen32b")
    ...
    patch_presentagent_for_text_qwen(args.language)
    modules = {
        "Config": Config,
        "Document": Document,
